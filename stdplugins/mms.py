...
async def _(event):
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
          ...
          if "Okay..." in response.text:
            await event.edit("```NANI?! This is a gif! This will take sum tym owo```")
            with BytesIO(file) as mem_file, BytesIO() as gif:
                resize_image(mem_file, gif)
                gif.seek(0)
                uploaded_gif = await borg.upload_file(gif, file_name=file_ext_ns_ion)
